  ...
  const ask = await inquirer.prompt<MevOnParam1>([
    {
      ...
      name: 'mevOn',
      message:
        'Do you want to enable solc Auto Operation Mode?(You can change it again)',
      default: false,
    },
  ])
  if (!ask.mevOn) {
...
If you enable this, solc will update automatically when the solc/solana version is updated.

AUTO UPDATE: If you enable this, solc will update automatically when the solc version is updated.

AUTO RESTART: If you enable this, solc will restart automatically when the solana version is updated.
