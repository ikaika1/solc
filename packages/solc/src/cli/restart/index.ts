  ...
  program
    ...
    .action(async (options: { rm: boolean }) => {
      if (options.rm) {
        ...
        spawnSync('solc stop', { stdio: 'inherit', shell: true })
        spawnSync('solc rm:snapshot', { stdio: 'inherit', shell: true })
        spawnSync('solc get snapshot', { stdio: 'inherit', shell: true })
        spawnSync('solc start', { stdio: 'inherit', shell: true })
