...
// It is defined in packages/solc/src/cli/config/config.ts
...
const autoUpdate = async (config: DefaultConfigType) => {
  ...
  const msg = `=== ✨ solc updated to the latest version ✨ ===
Validator Address: ${address}
solc Version: ${getSolvVersion()}
...
`
    await sendDiscord(msg)
    try {
      spawnSync(`solc update -b`, { stdio: 'inherit', shell: true })
    } catch (error) {
