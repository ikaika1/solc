...
export const updateCommands = (config: DefaultConfigType) => {
  ...
  program
    ...
    .option('--migrate-config', 'Migrate Solc Config', false)
    .option('--config', 'Update Solc Config Default Solana Version', false)
    ...
    .action(async (options: UpdateOptions) => {
      ...
      console.log(chalk.white(`Current solc version: ${solvVersion}`))
      ...
      if (options.migrateConfig) {
        // Temporarily!!
        // Migrate solc.config.json to solc4.config.json
      ...
      if (options.config) {
        ...
        console.log(
          chalk.green(
            '✔️ Updated Solc Config Default Solana Version\n\n You can now run `solc i` to install the latest version',
          ),
        )
        return
      }
