...
export const changeIdentityOutgoing = async (
  ...
  const step4 = `scp ${LEDGER_PATH}/tower-1_9-${pubkey}.bin solc@${ip}:${LEDGER_PATH}`
  ...
  console.log(chalk.white('🟢 Setting identity on the new validator...'))
  const cmd5 = `ssh -i ${sshKeyPath} -o StrictHostKeyChecking=no solc@${ip} -p 22 'cd ~ && source ~/.profile && ${step5}'`
  ...
  const cmd6 = `ssh -i ${sshKeyPath} -o StrictHostKeyChecking=no solc@${ip} -p 22 'cd ~ && source ~/.profile && ${step6}'`
